...
public class UserRegisterServiceImpl implements UserRegisterService {
	...
	public void registerNewUser(UserRegisterDto userRegisterDto) throws UserAlreadyExistsException {
		...
		String htmlContent = String.format(/* ... 郵件內容 ... */
				"<html><body>" + "<p>親愛的 %s,</p>" + "<p>感謝您註冊 零件傳承坊！請點擊以下連結來驗證您的帳戶：</p>" + // 請替換
						"<p><a href=\"%s\">%s</a></p>" + "<p>如果您沒有請求此驗證，請忽略此郵件。</p>" + "<p>謝謝，<br/>零件傳承坊 團隊</p>" + // 請替換
						"</body></html>",
	...
	public void resendEmail(String email) throws UserNotFoundException, IllegalStateException {
		...
		if (user.getLastEmailSentAt() != null
				&& ChronoUnit.SECONDS.between(user.getLastEmailSentAt(), LocalDateTime.now()) < RESEND_COOLDOWN_SECONDS) {
			long secondsRemaining = RESEND_COOLDOWN_SECONDS
					- ChronoUnit.SECONDS.between(user.getLastEmailSentAt(), LocalDateTime.now());
		...
		String htmlContent = String.format(
				"<html><body>" + "<p>親愛的 %s,</p>" + "<p>您請求了重新發送帳戶驗證郵件。請點擊以下連結進行驗證：</p>" + "<p><a href=\"%s\">%s</a></p>"
						+ "<p>如果您沒有請求此驗證，請忽略此郵件。</p>" + "<p>謝謝，<br/>零件傳承坊 團隊</p>" + "</body></html>",
				user.getUsername(), verificationLink, verificationLink);
		emailService.sendHtmlEmail(user.getEmail(), emailSubject, htmlContent);
	}
