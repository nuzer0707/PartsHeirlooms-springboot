...
public class ProfileController {
	...
	public ResponseEntity<ApiResponse<UserProfileDto>> getMyProfile(HttpSession session) {
		...
		try {
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body(ApiResponse.error(HttpStatus.INTERNAL_SERVER_ERROR.value(), "查詢個人資料失敗，請稍後再試"));
		}
	...
	// 使用者修改密碼
	@PutMapping("/password")
	private ResponseEntity<ApiResponse<Void>> changePassword(@Valid @RequestBody UserPasswordChangeDto passwordChangeDto,
			HttpSession session) {
	...
	public ResponseEntity<ApiResponse<Void>> addFavorite(@PathVariable Integer productId, HttpSession session) {
		UserCert userCert = (UserCert) session.getAttribute("userCert");
		if (userCert == null)
			return unauthorizedResponse("請先登入以查看收藏商品");
	...
	public ResponseEntity<ApiResponse<Boolean>> checkFavorite(@PathVariable Integer productId, HttpSession session) {
		UserCert userCert = (UserCert) session.getAttribute("userCert");
		if (userCert == null) {
	...
	public ResponseEntity<ApiResponse<SellerVerificationDto>> getMySellerApplicationStatus(HttpSession session) {
		...
		if (userCert == null)
			return unauthorizedGenericResponse("請先登入以查看賣家申請狀態");
	...
	public ResponseEntity<ApiResponse<SellerVerificationDto>> applyForSeller(
			@Valid @RequestBody SellerVerificationApplyDto applyDto, HttpSession session) {

		UserCert userCert = (UserCert) session.getAttribute("userCert");

		if (userCert == null)
			return unauthorizedGenericResponse("請先登入以申請成為賣家");
	...
	public ResponseEntity<ApiResponse<IssueReportDto>> submitIssueReport(
			@Valid @RequestBody IssueReportSubmitDto submitDto, HttpSession session) {
		UserCert userCert = (UserCert) session.getAttribute("userCert");
		if (userCert == null)
			return unauthorizedGenericResponse("請先登入以提交問題回報");
	...
	public ResponseEntity<ApiResponse<Boolean>> hasRatedTransaction(@PathVariable Integer transactionId,
			HttpSession session) {
		UserCert userCert = (UserCert) session.getAttribute("userCert");
		if (userCert == null) {
	...
	@PostMapping("/ratings")
	public ResponseEntity<ApiResponse<RatingDto>> submitRating(@Valid @RequestBody RatingSubmitDto ratingSubmitDto,
			HttpSession session) {
		UserCert userCert = (UserCert) session.getAttribute("userCert");

		if (userCert == null)
			return unauthorizedGenericResponse("請先登入以提交評價");
	...
	public ResponseEntity<ApiResponse<List<TransactionDto>>> getMyCurrentOrders(HttpSession session) throws UserNotFoundException {
		...
		//try {
			...
			if (transactionDtos.isEmpty()) {
				return ResponseEntity.ok(ApiResponse.success("您目前沒有未完成的訂單", transactionDtos));
			}
	...
	public ResponseEntity<ApiResponse<TransactionDto>> cancelMyOrder(@PathVariable Integer transactionId,
		...
		UserCert userCert = (UserCert) session.getAttribute("userCert");
		if (userCert == null) {
			return unauthorizedGenericResponse("請先登入以取消訂單");
		}
		try {
			TransactionDto cancelledTransaction = transactionService.buyerCancelTransaction(transactionId,
					userCert.getUserId());
			return ResponseEntity.ok(ApiResponse.success("訂單已成功取消", cancelledTransaction));
		} catch (UserNotFoundException e) {
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body(ApiResponse.error(HttpStatus.INTERNAL_SERVER_ERROR.value(), "系統錯誤：" + e.getMessage()));
		} catch (ProductNotFoundException e) {
	...
	public ResponseEntity<ApiResponse<TransactionDto>> getMyTransactionDetails(@PathVariable Integer transactionId,
		...
		UserCert userCert = (UserCert) session.getAttribute("userCert");
		if (userCert == null) {
			return unauthorizedGenericResponse("請先登入以查看交易詳情");
		}
		try {
			...
			return ResponseEntity.ok(ApiResponse.success("查詢查看交易詳情成功", transactionDto));
		} catch (ProductNotFoundException e) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND)
					.body(ApiResponse.error(HttpStatus.NOT_FOUND.value(), e.getMessage()));
		} catch (AccessDeniedException e) {
